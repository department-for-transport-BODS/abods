...
const failureResponse = (error: string): UserMutationResponse => ({
  error,
  success: false,
});
...
export class UserService {
  ...
  signUp$(
    ...
    return this.signUpMutation
      .mutate({ key, password, firstName, lastName })
      .pipe(
        map(
          ({ data, errors }) => data?.signUp ?? mutationFailureResponse(errors),
        ),
  ...
  requestResetPassword$(email: string): Observable<UserMutationResponse> {
    return this.requestResetPasswordMutation.mutate({ email }).pipe(
      map(
        ({ data, errors }) =>
          data?.requestResetPassword ?? mutationFailureResponse(errors),
      ),
      catchError(() => of(serverFailureResponse)),
    );
  }
  ...
  resetPassword$(
    ...
    return this.resetPasswordMutation
      .mutate({ uid, token, password, confirmPassword })
      .pipe(
        map(
          ({ data, errors }) =>
            data?.resetPassword ?? mutationFailureResponse(errors),
        ),
        catchError(() => of(serverFailureResponse)),
      );
  }
