...
describe("ForgottenPasswordComponent", () => {
  ...
  it("should show message if form submitted without email", () => {
    spyOn(service, "requestResetPassword$").and.returnValue(
      of({ success: true }),
    );
    ...
    expect(
      spectator.query(byText("Please enter a valid email address.")),
    ).toBeTruthy();
  });
  ...
  it("should call graphql with email", async () => {
    ...
    const requestResetPassword = controller.expectOne(
    ...
    expect(requestResetPassword.operation.variables).toEqual(
      jasmine.objectContaining({ email }),
    );
    controller.verify();
  });
