...
describe("SignupComponent", () => {
  ...
  it("should fetch invitation with the key in the url.", () => {
    spyOn(service, "invitation$").and.returnValue(
      of({ email: "foo@bar.com", accepted: false }),
    );
  ...
  it("should show message if invitation is already accepted.", async () => {
    spyOn(service, "invitation$").and.returnValue(
      of({ email: "foo@bar.com", accepted: true }),
    );
  ...
  [" ", "   ", "1029292", '"%&$%"$%'].forEach((testCase) => {
    it(`should not accept "${testCase}" as firstName`, () => {
      ...
      expect(
        spectator.query(byText(/Name contains an invalid character/)),
      ).toExist();
    });
  });

  ["Kevin", "Jérémie François", "     Timothy    ", "麗"].forEach(
    (testCase) => {
