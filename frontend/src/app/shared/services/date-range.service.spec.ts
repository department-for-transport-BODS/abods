...
describe("DateRangeService", () => {
  ...
  describe("calculatePresetPeriod", () => {
    it("last 7 days should calculate with comparison period correctly", () => {
      ...
      expect(
        spectator.service.calculatePresetPeriod(Period.Last7, testNow),
      ).toEqual(
    ...
    it("month to date should calculate with comparison period correctly", () => {
      ...
      expect(
        spectator.service.calculatePresetPeriod(Period.MonthToDate, testNow),
      ).toEqual(
        jasmine.objectContaining({
          from: startOfMonth,
          to: testNow.startOf("day"),
    ...
    it("should calculate month to date on 1st of the month sensibly", () => {
      ...
      const { from, to } = spectator.service.calculatePresetPeriod(
        Period.MonthToDate,
        testToday,
      );
    ...
    it("last month should calculate with comparison period correctly", () => {
      ...
      expect(
        spectator.service.calculatePresetPeriod(Period.LastMonth, testNow),
      ).toEqual(
        jasmine.objectContaining({
    ...
    it("last 28 days should be the default and calculate with comparison period correctly", () => {
      ...
      const twentyEightDaysBefore = DateTime.fromISO(
        "2021-02-03T10:01:00Z",
      ).startOf("day");
      const fiftySixDaysBefore = DateTime.fromISO(
        "2021-01-06T10:01:00Z",
      ).startOf("day");

      expect(
        spectator.service.calculatePresetPeriod(Period.Last28, testNow),
      ).toEqual(
        jasmine.objectContaining({
