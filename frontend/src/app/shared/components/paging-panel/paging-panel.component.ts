...
export class PagingPanelComponent {
  ...
  paginationChanged({ api }: PaginationChangedEvent) {
    ...
    this.lastRow =
      this.total === 0
        ? 0
        : Math.min(this.firstRow + pageSize - 1, this.rowCount);

    this.isFirstPage = this.current === 0;
    this.isLastPage = this.current === this.total - 1;

    this.stickToStart =
      this.total <= MINIMUM_PAGES || this.current < STICKY_THRESHOLD;
    this.stickToEnd =
      this.total <= MINIMUM_PAGES ||
      this.total - this.current - 1 < STICKY_THRESHOLD;
    ...
    const numPages =
      this.total <= MINIMUM_PAGES
        ...
        : Math.min(
            this.total,
            this.stickToStart || this.stickToEnd ? PAGES_STICKY : PAGES_MIDDLE,
          );
