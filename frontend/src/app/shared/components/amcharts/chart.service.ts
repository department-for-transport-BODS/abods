...
export const messageScreen = (
  ...
  indicatorLabel.dy = 50;
  indicatorLabel.fill =
    severity === "error" ? chartColors.red : chartColors.legendaryGrey;
  ...
  getErrorObject(errorScreen?: am4core.Container) {
    if (errorScreen) {
      messageScreen(
        errorScreen,
        "error",
        "There was an error loading the data for this chart, please try again.",
      );
    }
  }
  ...
  getNoDataObject(noDataScreen?: am4core.Container) {
    if (noDataScreen) {
      messageScreen(
        noDataScreen,
        "warn",
        "No data exists with the applied filters",
      );
    }
  }
