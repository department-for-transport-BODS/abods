...
describe("AdminAreaService", () => {
  ...
  it("should fetch admin areas", waitForAsync(() => {
  ...
  it("should fetch admin area boundaries", waitForAsync(() => {
    spectator.service.fetchAdminAreaBoundaries().subscribe((actual) => {
      ...
      expect(actual.features[0].geometry.coordinates).toEqual([
        DERBYSHIRE_BOUNDARY_SHAPE,
      ]);
    });
        ...
        adminAreas: [
          {
            ...
            shape: JSON.stringify(
              flip(geometry("Polygon", [DERBYSHIRE_BOUNDARY_SHAPE])),
            ),
          },
          {
            id: "AA370",
            name: "South Yorkshire",
            shape: JSON.stringify(geometry("Polygon", [])),
          },
          {
            id: "AA910",
            name: "National - National Rail",
  ...
  describe("fetchAdminAreasForOperator", () => {
    it("should return empty array if operator is empty string", waitForAsync(() => {
      spectator.service.fetchAdminAreasForOperator("").subscribe((data) => {
        expect(data).toEqual([]);
      });
    }));

    it("should return empty array if operator not found", waitForAsync(() => {
      spectator.service.fetchAdminAreasForOperator("ZZZ").subscribe((data) => {
        expect(data).toEqual([]);
      });
    }));

    it("should return empty array if operator found but no admin areas", waitForAsync(() => {
      spectator.service.fetchAdminAreasForOperator("AAA").subscribe((data) => {
        expect(data).toEqual([]);
      });
    }));

    it("should return array of admin areas if operator found", waitForAsync(() => {
      const expected = [
        {
          name: "Area 1",
          id: "1",
