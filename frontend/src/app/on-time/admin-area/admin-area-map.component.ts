...
@Component({
  ...
  >
    ...
    <mgl-layer
      ...
      [filter]="
        adminAreaIds.length
          ? ['in', ['get', 'id'], ['literal', adminAreaIds]]
          : ['has', 'id']
      "
      source="boundaries"
    ...
    >
      <div
        class="govuk-body-small govuk-!-font-weight-bold govuk-!-margin-bottom-1"
      >
...
export class AdminAreaMapComponent implements OnInit {
  ...
  recalculateBounds() {
    ...
    const selected = this.adminAreas.features.filter((feature) =>
      this._adminAreaIds?.includes(feature.properties.id),
    );
    if (!selected.length) {
      return;
    }
    this.bounds = combineBounds(
      selected?.map((feature) => feature.bbox as BBox2d),
    );
  }
