...
describe("ViewServiceComponent", () => {
  ...
  it("should show line data where a lineId is specified", waitForAsync(async () => {
    const spy = spyOn(onTimeService, "fetchServiceInfo").and.returnValue(
      of({
        serviceName: "Sheffield to Mansfield",
        serviceNumber: "53",
        serviceId: "XYZ",
      }),
    );
    ...
    expect(
      spectator.query(byText("53 - Sheffield to Mansfield")),
    ).toBeVisible();
  }));
  ...
  it("should not show mode selector for services that do not run frequently", async () => {
    ...
    ).and.returnValue(of({ numHours: 0, totalHours: 100 }));
  ...
  it("should show mode selector for services that do run frequently", async () => {
    ...
    const spy = spyOn(
      headwayService,
      "fetchFrequentServiceInfo",
    ).and.returnValue(of({ numHours: 50, totalHours: 100 }));
  ...
  it("should show summary of frequent service", async () => {
    ...
    spyOn(headwayService, "fetchFrequentServiceInfo").and.returnValue(
  ...
  it("should not call fetchOverviewStats with adminAreaIds", async () => {
    ...
    expect(performanceService.fetchOverviewStats).toHaveBeenCalledWith(
      expectedParams,
    );
  });
});
