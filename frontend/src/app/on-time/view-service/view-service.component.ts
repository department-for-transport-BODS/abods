...
export class ViewServiceComponent implements OnInit, OnDestroy {
  ...
  overviewMode: "on-time-performance" | "excess-wait-time" =
    "on-time-performance";
  ...
  ngOnInit(): void {
    ...
    paramMap$
      ...
      .subscribe(([service, queryParamMap]) => {
        ...
        if (!queryParamMap.has("tab"))
          this.tabs?.openTab("map", { emit: false });
      });
    ...
    const frequent$ = this.params$.pipe(
      ...
      switchMap((params) =>
        this.headwayService.fetchFrequentServiceInfo(params),
      ),
