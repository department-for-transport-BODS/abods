...
export class OnTimeService {
      ...
      .valueChanges.pipe(
        ...
        map((overview: PunctualityTotalsType) => {
          return {
            ...
            completed:
              overview.completed ||
              overview.early + overview.onTime + overview.late,
            noData:
  ...
  fetchOnTimeDelayFrequencyData(
    params: PerformanceParams,
  ): Observable<DelayFrequencyType[]> {
    return this.delayFrequencyDataQuery
      ...
      .pipe(
        map(({ data }) =>
          this.fillDelayFrequencyGaps(
            assert(data?.onTimePerformance?.delayFrequency),
          ),
        ),
      );
  }
  ...
  fetchOnTimePerformanceList(
    params: PerformanceParams,
  ): Observable<ServicePerformance[]> {
  ...
  fetchOperatorPerformanceList(
    params: PerformanceInputType,
  ): Observable<OperatorPerformance[]> {
  ...
  fetchServiceInfo(lineId: string): Observable<ServiceInfoType> {
    return this.serviceInfoQuery
      .fetch({ lineId }, { fetchPolicy: "no-cache", errorPolicy: "none" })
      .pipe(
  ...
  fillDelayFrequencyGaps(data: DelayFrequencyType[]): DelayFrequencyType[] {
    ...
    return init.reduceRight(
      ([ahead, ...acc], value) => {
        const filler = _range(value.bucket + 1, ahead.bucket).map((bucket) => ({
          bucket,
          frequency: 0,
        }));
