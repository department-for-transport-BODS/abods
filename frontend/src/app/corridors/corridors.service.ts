...
const formatDayOfWeekShort = (weekday: number) =>
  DateTime.fromObject({ weekday }).toFormat("ccc");
const formatDayOfWeek = (weekday: number) =>
  DateTime.fromObject({ weekday }).toFormat("cccc");
...
export class CorridorsService {
  ...
  fetchSubsequentStops(stopList: string[]): Observable<CorridorStop[]> {
    return this.corridorsSubsequentStopsQuery
      ...
      .pipe(
        map((result) =>
          nonNullishArray(result?.data?.corridor?.addSubsequentStops).map(
            toStop,
          ),
        ),
      );
  ...
  fetchCorridorById(corridorId: number): Observable<Corridor> {
    return this.getCorridorGQL
      .fetch({ corridorId }, { fetchPolicy: "no-cache" })
      .pipe(
        map((result) => {
        ...
        assertNonNullish(),
        map(({ stops, ...corridor }) => ({
          stops: nonNullishArray(stops).map(toStop),
          ...corridor,
        })),
      );
  }
