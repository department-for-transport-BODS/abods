...
describe("CreateCorridorComponent", () => {
  ...
  describe("create new corridor", () => {
    ...
    it("should search for stops", async () => {
      const spy = spyOn(corridorsService, "queryStops").and.returnValue(
        of({ orgStops: [testStop1], nonOrgStops: [] }),
      );
      ...
      spectator.selectOption(
        byLabel("Search for the first stop in your corridor"),
        "stop",
      );
    ...
    it("should save corridor", async () => {
      ...
      expect(serviceSpy).toHaveBeenCalledWith("test corridor", [
        "ST012345",
        "ST023456",
      ]);
    ...
    it("should prevent duplicate submissions", async () => {
      ...
      expect(serviceSpy).toHaveBeenCalledOnceWith("duplicate corridor", [
        "ST012345",
        "ST023456",
      ]);
    ...
    it("should show empty list if user deletes search query", async () => {
      ...
      spectator.typeInElement(
        "",
        byLabel("Search for the first stop in the corridor"),
      );
    ...
    it("should display custom error message if submit call errors", () => {
      spyOn(corridorsService, "createCorridor").and.returnValue(
        throwError(() => "error"),
      );
    ...
    it("should search for stops by location", async () => {
      ...
      const spy = spyOn(corridorsService, "queryStops").and.returnValue(
        of({ orgStops: [testStop4], nonOrgStops: [] }),
      );
      ...
      spectator.typeInElement(
        "eckington",
        byLabel("Location name or postcode"),
      );
      await spectator.fixture.whenStable();
      spectator.detectChanges();

      spectator.click(
        byTextContent("Eckington Sheffield", { selector: ".ng-option" }),
      );
      // Simulate map move event
      spectator.component.corridorMap.boundsChanged.emit(
        new LngLatBounds(
          [-1.387216112, 53.293773104],
          [-1.334991807, 53.317192882],
        ),
      );
      await spectator.fixture.whenStable();
      spectator.detectChanges();
    ...
    it("should search for stops by location when map moves", async () => {
      const spy = spyOn(corridorsService, "queryStops").and.returnValue(
        of({ orgStops: [testStop3], nonOrgStops: [] }),
      );
    ...
    it("should clear hover state when loading new stops", async () => {
      ...
      spyOn(corridorsService, "queryStops").and.returnValue(
        of({ orgStops: [testStop1], nonOrgStops: [] }),
      );
    ...
    it("centreMapBounds() should call setMapBounds() if currentBounds are present", () => {
      ...
      expect(spy).toHaveBeenCalledWith(
        spectator.component.currentBounds,
        FIT_BOUNDS_OPTIONS,
      );
    });
  ...
  describe("edit corridor", () => {
    ...
    it("should show error message if delete corridor errored", async () => {
      spyOn(corridorsService, "deleteCorridor").and.returnValue(
        throwError(() => "error"),
      );
    ...
    it("should show error message if update corridor errored", async () => {
      spyOn(corridorsService, "updateCorridor").and.returnValue(
        throwError(() => "error"),
      );
