...
export class SegmentSelectorComponent implements OnChanges, AfterViewInit {
  ...
  get isServiceLinksLoaded(): boolean {
    return (
      isNotNullOrUndefined(this.serviceLinks) && this.serviceLinks.length > 0
    );
  }

  get showScrollControls(): boolean {
    return (
      this.scrollContent?.nativeElement.offsetWidth <
  ...
  get last(): boolean {
    return (
      this.scrollEl.scrollLeft + this.scrollEl.offsetWidth ===
  ...
  containsInvalidServiceLink(): boolean {
    const segment = this.segments?.find((segment) =>
      this.isInvalidServiceLink(segment),
    );
  ...
  scrollRight() {
    this.scrollEl.scrollTo({
      left:
        ...
        (this.scrollEl.offsetWidth - (this.SEGMENT_MIN_WIDTH + this.STOP_SIZE)),
      behavior: "smooth",
    });
  }
