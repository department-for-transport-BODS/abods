...
import {
  createVehiclePingStop,
  createNoDataStop,
} from "./vehicle-ping-stop.model";
...
describe("VehiclePingStop", () => {
  ...
  it("should set actual departure by adding delay to scheduled departure", () => {
    ...
    expect(vps.actualDeparture?.toISO()).toEqual(
      DateTime.fromISO(startTime).toISO(),
    );

    ping.delay = 120;
    vps = createVehiclePingStop(ping, stop);

    expect(vps.actualDeparture).toEqual(
