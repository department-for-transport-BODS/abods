...
describe("VehicleJourneysViewComponent", () => {
  ...
  beforeEach(() => {
    ...
    spectator
      ...
      .fetchNextPrevJourneys.and.returnValue(
        of([mockPrevJourney, mockNextJourney]),
      );
  });

  it("should call getVehicleJourneyView with journeyId, startTime and viewParams", () => {
    const spy = spyOn(viewService, "getVehicleJourneyView").and.returnValue(
      of(mockView),
    );
  ...
  it("should display error message when getVehicleJourneyView throws an error", () => {
    spyOn(viewService, "getVehicleJourneyView").and.returnValue(
    ...
    expect(
      spectator.query(
        byTextContent("Not found", { selector: ".page-header__title" }),
      ),
    ).toBeVisible();
    expect(
      spectator.query(
