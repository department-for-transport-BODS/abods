...
fdescribe("CookieBannerComponent", () => {
  ...
  describe("submitted", () => {
    const createComponent = createComponentFactory({
      ...
      providers: [
        MockProvider(CookiePolicyService, mockCookiePolicyServiceSubmitted),
      ],
    });
  ...
  describe("unsubmitted", () => {
    const createComponent = createComponentFactory({
      ...
      mocks: [AnalyticsService],
      providers: [
        MockProvider(CookiePolicyService, mockCookiePolicyServiceUnsubmitted),
      ],
    });
    ...
    it("should show cookie banner with accept and reject buttons if not submitted", () => {
      expect(
        spectator.query(
          byText("We use some essential cookies to make this service work."),
        ),
      ).toBeVisible();
    ...
    it("should notify user has accepted analytics cookies", () => {
      ...
      expect(
        spectator.query(byText("Accept analytics cookies")),
      ).not.toBeVisible();
      expect(
        spectator.query(byText("Reject analytics cookies")),
      ).not.toBeVisible();
    });
