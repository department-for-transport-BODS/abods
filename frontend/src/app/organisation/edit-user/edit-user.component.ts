    ...
    this.subs.push(
      ...
      combineLatest([this.username, this.userList])
        .pipe(
          map(
            ([usernamet, userList]) =>
              userList.find(({ username }) => usernamet === username) ?? null,
          ),
        )
        .subscribe((u) => {
          this.user.next(u);
      ...
      this.service
        ...
        .subscribe((mures) => {
          if (mures.success) {
            ...
            this.errors = [
              {
                error: mures.error ?? "There was an issue submitting the form",
              },
            ];
    ...
    this.subs.push(
      this.service.removeUser$(this.username.value).subscribe((mures) => {
        if (mures.success) {
          ...
          this.errors = [
            { error: mures.error ?? "There was an issue deleting the user" },
          ];
          this.submitted = false;
