...
describe("EditUserComponent", () => {
  ...
  it("should should display details for user in list", async () => {
    ...
    expect(spectator.query(byLabel("First name"))).toHaveValue(
      person?.firstName ?? "",
    );
    expect(spectator.query(byLabel("Last name"))).toHaveValue(
      person?.lastName ?? "",
    );
    expect(spectator.query(byLabel("Email address"))).toHaveValue(
      person?.username ?? "",
    );
  ...
  it("should validate first name", async () => {
    ...
    const spy = spyOn(service, "editUser$").and.returnValue(
      of({ success: true }),
    );
    ...
    expect(
      spectator.query(
        byText("This field is required.", {
          selector: "#user-first-name-error",
        }),
      ),
    ).toBeTruthy();
  });
  ...
  it("should not navigate back to list but show error on save failure", async () => {
    ...
    spyOn(service, "editUser$").and.returnValue(
      of({ success: false, error: expectedError }),
    );
  ...
  it("should check that the user really wants to revoke access", async () => {
    ...
    const editSpy = spyOn(service, "editUser$").and.returnValue(
      of({ success: true }),
    );
    const removeSpy = spyOn(service, "removeUser$").and.returnValue(
      of({ success: true }),
    );
    ...
    expect(
      spectator.query(
        byText(
          new RegExp(
            `Are you sure you want to remove the account for ${username}`,
          ),
        ),
      ),
    ).toBeTruthy();
  ...
  it("should allow them to revoke access if the really want to", async () => {
    ...
    expect(navigateSpy.calls.mostRecent().args[0].toString()).toEqual(
      `/organisation/users`,
    );
  });
