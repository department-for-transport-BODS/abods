...
describe("EditAlertComponent", () => {
  ...
  it("should load feed failure event correctly", async () => {
    ...
    expect(
      spectator.query(
        byLabel("If vehicle data has been missing for more than"),
      ),
    ).toHaveValue(feedFailureAlert.eventHysterisis.toString());

    expect(
      spectator.query(byLabel("Send a notification to")),
    ).toHaveSelectedOptions(feedFailureAlert.sendTo.id);
  });
  ...
  it("should load vehicle count disparity event correctly", async () => {
    ...
    expect(
      spectator.query(byLabel("If the count disparity is greater than")),
    ).toHaveValue(vehicleDisparityAlert.eventThreshold.toString());

    expect(
      spectator.query(byLabel("Send a notification to")),
    ).toHaveSelectedOptions(vehicleDisparityAlert.sendTo.id.toString());
  });
  ...
  it("should validate alert type", () => {
    ...
    const createSpy = spyOn(service, "createUserAlert$").and.returnValue(
      of({ success: true }),
    );
  ...
  it("should not allow 0 eventThreshold", () => {
    ...
    spectator.typeInElement(
      "0",
      byLabel("If the count disparity is greater than"),
    );
  ...
  it("should update existing alert on save", async () => {
    ...
    const updateSpy = spyOn(service, "updateUserAlert$").and.returnValue(
      of({ success: true }),
    );
  ...
  it("should allow existing alert to be deleted", async () => {
    ...
    const deleteSpy = spyOn(service, "deleteUserAlert$").and.returnValue(
      of({ success: true }),
    );
  ...
  it("should display error message on failed update", async () => {
    ...
    spyOn(service, "updateUserAlert$").and.returnValue(
      of({ success: false, error: "Update alert error to be shown" }),
    );
    ...
    expect(
      spectator.query(byText("Update alert error to be shown")),
    ).toBeTruthy();
  });
  ...
  it("should not emit closeEdit after failed update", async () => {
    ...
    const closeEditSpy = spyOn(component.closeEdit, "emit");
    spyOn(service, "updateUserAlert$").and.returnValue(
      of({ success: false, error: "Update alert error" }),
    );
  ...
  it("should display error message on failed create", async () => {
    ...
    spyOn(service, "createUserAlert$").and.returnValue(
      of({ success: false, error: "Create alert error to be shown" }),
    );
    ...
    expect(
      spectator.query(byText("Create alert error to be shown")),
    ).toBeTruthy();
  });
  ...
  it("should not emit closeEdit after failed create", async () => {
    ...
    const closeEditSpy = spyOn(component.closeEdit, "emit");
    spyOn(service, "createUserAlert$").and.returnValue(
      of({ success: false, error: "Create alert error" }),
    );
  ...
  it("should display error message on failed delete", async () => {
    ...
    spyOn(service, "deleteUserAlert$").and.returnValue(
      of({ success: false, error: "Delete alert error to be shown" }),
    );
    ...
    expect(
      spectator.query(byText("Delete alert error to be shown")),
    ).toBeTruthy();
  });
