...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  operatorSubject = new BehaviorSubject<
    OperatorDashboardFragment | { nocCode: "all" }
  >({ nocCode: "all" });
  ...
  private feedStatusOperators$: Observable<OperatorDashboardFragment[]> =
    this.selectedOperatorsSubject.pipe(
  ...
  ngOnInit(): void {
    this.currentVehicles$
      ...
      .subscribe((cv) => (this.currentVehicles = cv));
    this.expectedVehicles$
      .pipe(takeUntil(this.onDestroy$))
      .subscribe((ev) => (this.expectedVehicles = ev));
    this.feedStatusOperators$
      .pipe(takeUntil(this.onDestroy$))
      .subscribe((fsos) => (this.feedStatusOperators = fsos));
    this.nocCodeSubject
      .pipe(takeUntil(this.onDestroy$))
      .subscribe((nocCode) => (this.nocCode = nocCode));
    ...
    combineLatest([
      this.nocCodeSubject,
      this.service.listOperatorVehicleCounts.pipe(share()),
    ])
      .pipe(
    ...
    const allStops$ = this.route.queryParamMap.pipe(
      map((queryParamMap) => queryParamMap.get("allStops") === "true"),
    );
    ...
    combineLatest([this.operatorSubject, allStops$])
      ...
      .subscribe(([operator, allStops]) => {
        ...
        this.performanceFilters$.next({
          ...
          operatorIds:
            operator.operatorId && operator.operatorId !== "all"
              ? [operator.operatorId]
              : undefined,
        });
      });
