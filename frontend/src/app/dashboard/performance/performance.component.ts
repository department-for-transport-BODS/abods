...
import {
  DateRangeService,
  WindowDatetimes,
} from "../../shared/services/date-range.service";
import {
...
export class PerformanceComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit(): void {
    combineLatest([this.filters, this.fromToSubject])
      .pipe(
        switchMap(([filters, { from, to }]) =>
          this.service.getPunctualityStats(filters, from, to),
        ),
        map((stats) => {
          if (
            stats?.success &&
    ...
    combineLatest([this.filters, this.fromToSubject, this.rankingOrderSubject])
      .pipe(
        ...
        switchMap(([filters, { from, to, trendFrom, trendTo }, order]) =>
          this.service.getServiceRanking(
            filters,
            from,
            to,
            order,
    ...
    this.fromToSubject
      ...
      .subscribe((fromTo) => (this.fromTo = fromTo));
  }

  ngAfterViewInit(): void {
    this.fromToSubject.next(
      this.dateRange.calculatePresetPeriod(Preset.Last7, DateTime.local()),
    );
  ...
  changePeriod(event: Event) {
    this.period =
      ((event.target as HTMLInputElement).value as Period) ?? Period.Last7;

    const now: DateTime = DateTime.local();

    return this.fromToSubject.next(
      this.dateRange.calculatePresetPeriod(this.period, now),
    );
  }
