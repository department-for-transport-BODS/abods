...
describe("PerformanceComponent", () => {
  ...
  it("should fetch stats for all operators for last month", () => {
    const getPunctualityStatsSpy = spyOn(
      service,
      "getPunctualityStats",
    ).and.returnValue(cold("-a", { a: fakeDashboardServiceRanking() }));
    const getServiceRankingSpy = spyOn(
      service,
      "getServiceRanking",
    ...
    const changePeriodFromTimeSpy = spyOn(
      dateRange,
    ...
    expect(changePeriodFromTimeSpy).toHaveBeenCalledWith(
      Period.LastMonth,
      DateTime.local(),
    );

    expect(getPunctualityStatsSpy).toHaveBeenCalledWith(
      filters,
      startOfLastMonth,
      endOfLastMonth,
    );
  ...
  it("should pass ranking to ranking component", () => {
    ...
    spyOn(service, "getPunctualityStats").and.returnValue(
      cold("-a", {
        a: { success: true, result: fakeDashboardServiceRanking() },
      }),
    );
    spyOn(service, "getServiceRanking").and.returnValue(
      cold("-a", { a: ranking }),
    );
