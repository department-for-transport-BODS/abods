...
const DistancesPage = () => {
  ...
  const adminAreaOptions = useMemo(() => {
    ...
    if (selectedOrgs.length > 0) {
      opIds = new Set(adminOrgData.filter(d => d.orgName !== null && selectedOrgs.includes(d.orgName)).map(d => d.operatorId));
    }
    ...
    return Array.from(new Set(
      adminOrgData.filter(d => relevantOpIds.has(d.operatorId)).map(d => d.adminName).filter((n): n is string => n !== null)
    )).sort();
  ...
  const orgOptions = useMemo(() => {
    ...
    if (selectedAdminAreas.length > 0) {
      opIds = new Set(adminOrgData.filter(d => d.adminName !== null && selectedAdminAreas.includes(d.adminName)).map(d => d.operatorId));
    }
    const relevantOps = (dropdownInputsData.operators ?? []).filter(op => {
      if (opIds && !opIds.has(op.id)) return false;
      if (selectedOperators.length > 0 && !selectedOperators.includes(`${op.name} (${op.id})`)) return false;
    ...
    return Array.from(new Set(
      adminOrgData.filter(d => relevantOpIds.has(d.operatorId)).map(d => d.orgName).filter((n): n is string => n !== null)
    )).sort();
  ...
  const operatorOptions = useMemo(() => {
    ...
    if (selectedOrgs.length > 0) {
      const orgOpIds = new Set(adminOrgData.filter(d => d.orgName !== null && selectedOrgs.includes(d.orgName)).map(d => d.operatorId));
  ...
  const handleGenerateDataButton = async () => {
    ...
    .filter((adminArea) => adminArea.adminName !== null && selectedAdminAreas.includes(adminArea.adminName))
        ...
        {error && (
          <div className="govuk-error-summary" role="alert" aria-labelledby="error-summary-title">
            ...
            <div className="govuk-error-summary__body">
              <p className="govuk-body">There was a problem loading the filter data. Please try refreshing the page.</p>
            </div>
          </div>
        )}
        {generateError && (
          <div className="govuk-error-summary" role="alert" aria-labelledby="generate-error-summary-title">
            <h2 className="govuk-error-summary__title" id="generate-error-summary-title">There is a problem</h2>
            <div className="govuk-error-summary__body">
              <p className="govuk-body">There was a problem fetching the distances data. Please try again.</p>
            </div>
          </div>
        )}
        <DistanceFilters
