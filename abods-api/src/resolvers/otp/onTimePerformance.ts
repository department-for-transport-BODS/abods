      ...
      if (operatorIds.length == 1) {
        ...
        if (userOperatorIds.includes(operator_noc_to_filter)) {
          ...
          let mainQuery = context.kysely
            ...
            .select((eb) => [
              sql<number>`SUM(${eb.ref("count_delayed")} * CAST(${eb.ref("average_delay")} AS integer))`.as(
                "average_delay",
              ),
              sql<number>`SUM(CAST(${eb.ref("avg_time_difference")} AS integer) * ${eb.ref("on_time_count")}) FILTER (WHERE ${eb.ref("on_time_count")} > 0) * 60`.as(
                "on_time_in_seconds",
              ),
              sql<number>`SUM(CAST(${eb.ref("avg_time_difference")} AS integer) * ${eb.ref("late_count")}) FILTER (WHERE ${eb.ref("late_count")} > 0) * 60`.as(
                "late_in_seconds",
              ),
              sql<number>`SUM(CAST(${eb.ref("avg_time_difference")} AS integer) * ${eb.ref("early_count")}) FILTER (WHERE ${eb.ref("early_count")} > 0) * 60`.as(
                "early_in_seconds",
              ),
