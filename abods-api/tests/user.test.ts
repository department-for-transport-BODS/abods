...
import {
  ...
  getUserAlerts,
  getUsers,
} from "../src/resolvers/userFunctions.js";
...
describe("User functionality testing", () => {
  test("should return multiple users", async () => {
    ...
    await expect((getUsers as any)({}, {}, { db })).resolves.toEqual(
      testOutputUsers,
    );
    expect(db.bods_user.findMany).toBeCalledTimes(1);
    expect(
      db.bods_user.findMany.mock.calls[0][0].where.userOrganisations.every
        .organisation_id.in,
    ).toEqual(mockUserRecord.orgIds);
  });
  ...
  test("get users alerts", async () => {
    ...
    await expect((getUserAlerts as any)({}, {}, { db })).resolves.toEqual(
      testOutputAlerts,
    );
  });
});
