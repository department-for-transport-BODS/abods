...
loggedInTest.describe("Feed Monitoring Page - Authenticated", () => {
  ...
  loggedInTest("renders live status page if operator clicked", async () => {
    ...
    await expect(feedMonitoring.page).toHaveURL(
      new RegExp(`/feed-monitoring/${nocCode}`),
    );
    ...
    // Wait for loading indicators to disappear
    await expect(feedMonitoring.page.getByText(/Loading\.\.\./i)).toHaveCount(
      0,
      { timeout: 60000 },
    );
...
test("Live Status Page - Unauthenticated", async ({ page }) => {
  let nocCode: string = process.env.TEST_NOC_CODE as string;
  await page.goto(`/feed-monitoring/${nocCode}`, {
...
loggedInTest.describe("Live Status Page - Authenticated", () => {
  ...
  loggedInTest("renders the feed history page if link clicked", async () => {
    ...
    let nocCode: string = process.env.TEST_NOC_CODE as string;
    await expect(liveStatus.page).toHaveURL(
      new RegExp(`/feed-monitoring/${nocCode}/feed-history`),
    );
    ...
    // Wait for loading indicators to disappear
    await expect(liveStatus.page.getByText(/Loading\.\.\./i)).toHaveCount(0, {
      timeout: 60000,
    });
  ...
  loggedInTest(
    "renders the feed monitoring page if back link clicked",
    async ({ loggedInPage }) => {
...
test("Feed History Page - Unauthenticated", async ({ page }) => {
  let nocCode: string = process.env.TEST_NOC_CODE as string;
  await page.goto(`/feed-monitoring/${nocCode}/feed-history`, {
    waitUntil: "domcontentloaded",
  });
  await expect(page).toHaveURL(/\/login/);
});
...
loggedInTest.describe("Feed History Page - Authenticated", () => {
  ...
  loggedInTest(
    "renders the previous and next link and shows correct page",
    async () => {
      await feedHistory.previousLink().click();

      let nocCode: string = process.env.TEST_NOC_CODE as string;
      await expect(feedHistory.page).toHaveURL(
        new RegExp(`/feed-monitoring/${nocCode}`),
      );
      ...
      // Wait for loading indicators to disappear
      await expect(feedHistory.page.getByText(/Loading\.\.\./i)).toHaveCount(
        0,
        { timeout: 60000 },
      );
      ...
      const expectedDateText = dayBeforeYesterday.toLocaleDateString("en-GB", {
        day: "numeric",
        month: "long",
        year: "numeric",
      });
      ...
      const expectedDateText2 = yesterday.toLocaleDateString("en-GB", {
        day: "numeric",
        month: "long",
        year: "numeric",
      });
  ...
  loggedInTest(
    "renders the back to live status link and navigates back to live status page",
    async () => {
